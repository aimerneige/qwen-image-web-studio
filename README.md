...
- **实时步进流式反馈（SSE）与随时中断取消**：基于 Server-Sent Events 实现毫秒级进度同步，页面实时显示当前推理阶段、去噪采样步数（Step X/Y）、进度百分比、已耗时与预估剩余时间（ETA）；支持**一键取消任务**，毫秒级中断去噪计算，自动清理 PyTorch 显存缓存并立即释放硬件独占锁。
...
- **SQLite 纯净持久化存储**：采用 Python 标准库内置 `sqlite3`（零额外依赖），任务完成后将输入参考图、生成图片、完整提示词、负向词、随机种子、推理步数、生成耗时（秒）及创建时间完整存入本地 `history.db`；彻底废弃无 prompt 旧文件扫描回退，仅展示纯净真实的历史数据，服务重启数据永不丢失。
- **便捷工作流与历史记录**：
